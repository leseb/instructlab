...
def ensure_server(
    ...
    try:
        ...
        if serve_config.backend == "vllm":
            backend_server = backends.VllmServer(
                logger,
                temp_api_base,
                serve_config.model_path,
                host,
                port,
                serve_config.vllm_args,
            )
        else:
            backend_server = backends.LlamaServer(
                logger,
                temp_api_base,
                serve_config.model_path,
                host,
                port,
                serve_config.gpu_layers,
                serve_config.max_ctx_size,
                model_family,
            )
